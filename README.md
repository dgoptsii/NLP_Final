...
- **[Nazario Phishing Corpus](https://monkey.org/~jose/phishing/)**: real-world phishing emails. Place the `.mbox` file in `data/raw/mbox/`.
- **[Enron Email Dataset](https://www.cs.cmu.edu/~enron/)**: legitimate corporate emails. Place the `maildir` folder in `data/raw/maildir/`.
...
**Step 1 — Preprocessing and build dataset splits:**
```bash
...
**Step 2 — Generate obfuscated test set:**
```bash
python scripts/obfuscate.py
```
 
**Step 3 — Tune and train models:**
```bash
...
**Step 5 — Run stress tests (obfuscation robustness):**
```bash
...
**Step 6 — Run full transformer pipeline (training, stress test, and LIME):**
```bash
